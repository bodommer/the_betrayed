...
public class Armoury {
		...
		while (true) {
			...
			if (attr.containsKey(input)) {
				if (hero.getCoins() >= attr.get(input)) {
					...
					System.out.printf("You have successfully bought a new shiny piece of armour! You now have %d coins.", hero.getCoins());
					return;
				} else {
	...
	public static void main(String[] args) {
		System.out.println(((double) Math.round(0.12554534544444 * 100))/100);
		Armoury.shop(new Hero("GLorious", "att"), new Scanner(System.in));
	}
}
